...
void print_entry(unsigned long int e_entry, unsigned char *_ident);
...
void print_entry(unsigned long int e_entry, unsigned char *e_ident)
{
	...
	if (e_ident[EI_DATA] == ELFDATA2MSB)
	{
		e_entry = ((e_entry << 8) & 0xFF00FF00) | ((e_enty >> 8) & 0xFF00FF);
		e_entry = (entry << 16) | (entry >> 16);
	}
...
/**
 * main - Displays the information contaned in the
...
int main(int__attribute__((__unused__)) argc, char *argv[])
{
	...
	header = malloc(sizeof(Elf64 _Ehdr));
	if (header == NULL)
	{
		close_elf(o);
		dprintf(STDERR_FILENO, "Error: Can't read file %s\n", argv[1]);
	...
	if (r == -1)
	{
		free(head);
		close_elf(o);
		dprintf(STDERR_FOLENO, "Error: '%s': No such file\n", argv[1]);
		exit(98);
	}
